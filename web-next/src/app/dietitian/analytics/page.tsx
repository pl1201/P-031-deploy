    ...
    <div className={styles.dashboard}>
      <main>
        ...
        <section className={styles.twoPanels}>
          ...
          <section className={styles.panel}><PanelTitle step="04" eyebrow="CHẤT LƯỢNG DỮ LIỆU" title="Khả năng đưa ra kết luận" note="Dữ liệu thiếu được tách khỏi kết quả đạt."/><div className={styles.donutRow}><div className={styles.donut} style={{background:`conic-gradient(#75c89c 0% ${model.dataReady}%, #e6aa45 ${model.dataReady}% 100%)`}}><span><strong>{model.dataReady}%</strong><small>sẵn sàng</small></span></div><div className={styles.legend}><p><i style={{background:'#75c89c'}}/><span>Không có dòng chờ</span><strong>{Math.max(0,patients.length-model.attentionPatients)}</strong></p><p><i style={{background:'#e6aa45'}}/><span>Cần đối chiếu</span><strong>{model.attentionPatients}</strong></p></div></div></section>
        </section>
...
function buildModel(plans:MealPlan[],patients:PatientProfile[],logs:FoodLog[],now:number){
  ...
  const riskDefs=[['P0','Chặn phát hành','#dc6a58'],['P1','Cần xác nhận','#e6aa45'],['P2','Lưu ý','#5db8b5'],['none','Đã qua kiểm tra','#75c89c']] as const;const risks=riskDefs.map(([key,label,color])=>({label,color,count:plans.filter(p=>p.highest_risk===key).length}));const riskTotal=risks.reduce((s,r)=>s+r.count,0);let cursor=0;const stops=risks.map(r=>{const start=cursor;cursor+=riskTotal?r.count/riskTotal*100:0;return`${r.color} ${start}% ${cursor}%`});const riskGradient=`conic-gradient(${stops.join(',')||'#294242 0 100%'})`
