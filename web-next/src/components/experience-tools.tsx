...
function DoctorMascot({className}:{className?:string}){
  return <svg className={className} viewBox="0 0 48 48" aria-hidden="true">
    ...
    <path d="M16 20c0-6 3.6-10 8-10s8 4 8 10v3H16z" fill="currentColor" opacity=".2"/>
    <circle cx="24" cy="20" r="7" fill="#f3c9a8"/>
    <path d="M17.5 18.5c.4-5.2 3.1-8.3 6.7-8.3 4 0 6.2 3.2 6.5 7.2-2.5-.5-4.4-1.8-5.7-3.8-1.8 2.4-4.2 4-7.5 4.9Z" fill="#173f56"/>
    <circle cx="21.5" cy="20.5" r=".8" fill="#173f56"/><circle cx="26.5" cy="20.5" r=".8" fill="#173f56"/>
    <path d="M21.5 23.5c1.6 1.3 3.4 1.3 5 0" fill="none" stroke="#9b5f52" strokeWidth="1.2" strokeLinecap="round"/>
    <path d="M13 40c.7-8.1 4.6-12 11-12s10.3 3.9 11 12" fill="#fff" stroke="currentColor" strokeWidth="1.8"/>
    <path d="m20 29 4 5 4-5M24 34v6" fill="none" stroke="#1784b8" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M18.5 31.5c-3.5 4.2-1.4 8.2 2.3 7.4M29.5 31.5c3.5 4.2 1.4 8.2-2.3 7.4" fill="none" stroke="#173f56" strokeWidth="1.4" strokeLinecap="round"/>
    <circle cx="20.5" cy="38.5" r="1.8" fill="#fff" stroke="#173f56" strokeWidth="1.2"/>
  </svg>
}
...
const KNOWLEDGE:Array<{keys:RegExp;answer:string;suggestions:string[]}>= [
  ...
  {keys:/(nguoi benh|benh nhan|hom nay|mon da an)/,answer:'Không gian người bệnh gồm 4 khu vực: Hôm nay để xem thực đơn đã phát hành và ghi nhận nhanh; Tuần của tôi để ghi món theo từng ngày/bữa và xem mức độ đầy đủ dữ liệu; Báo cáo dinh dưỡng để xem xu hướng đã ăn thực tế so với ngưỡng; Hồ sơ để kiểm tra thông tin đang được dùng. Người bệnh không nhìn thấy bản nháp chưa được chuyên gia duyệt.',suggestions:['Cách ghi món đã ăn','Vì sao chưa có thực đơn?','Cập nhật hồ sơ']},
  ...
  {keys:/(nhat ky|ghi mon|mon da an|khau phan|gram)/,answer:'Mở Tuần của tôi, bấm vào ô ngày/bữa muốn ghi, nhập tên món quen thuộc và khẩu phần nếu biết. Nếu không nhớ chính xác gram, bạn có thể để trống; hệ thống sẽ không tự bịa số liệu. Món chưa khớp được đưa vào hàng chờ để chuyên gia đối chiếu.',suggestions:['Món chưa đối chiếu là gì?','Xem tổng hợp tuần','Báo bỏ bữa']},
  {keys:/(tuan cua toi|tong hop tuan|bao cao tuan|bao cao dinh duong)/,answer:'Trang Tuần của tôi dùng để ghi món theo từng ngày/bữa, so sánh với thực đơn đã duyệt. Trang Báo cáo dinh dưỡng tổng hợp các món đã ăn thực tế thành xu hướng theo dưỡng chất (natri, xơ, tinh bột…) so với ngưỡng — chỉ để xem, không ghi món ở đây. Khi dữ liệu chưa đủ, hệ thống hiển thị rõ “chưa tính được” thay vì kết luận đạt hay không đạt.',suggestions:['Bao nhiêu ngày là đủ?','Món chờ đối chiếu','Ghi bữa hôm nay']},
...
function screenHint(path:string){if(path.includes('/meal-plans/new'))return 'Bạn đang ở màn hình tạo thực đơn; mình có thể hướng dẫn chọn hồ sơ, tạo phương án hoặc đọc kiểm tra.';if(path.includes('/food-logs'))return 'Bạn đang ở màn hình đối chiếu nhật ký; mình có thể giải thích món chờ và cách xử lý.';if(path.includes('/reviews'))return 'Bạn đang ở hàng chờ duyệt; mình có thể hướng dẫn xem bằng chứng, duyệt hoặc từ chối.';if(path.includes('/patient/diary'))return 'Bạn đang ở Báo cáo dinh dưỡng; mình có thể giải thích các chỉ số. Muốn ghi món, hãy mở Tuần của tôi.';if(path.includes('/patient/weekly'))return 'Bạn đang ở Tổng hợp tuần; mình có thể giải thích từng chỉ số.';if(path.includes('/patient/profile'))return 'Bạn đang ở Hồ sơ; mình có thể giải thích dữ liệu và yêu cầu cập nhật.';return 'Bạn có thể cho mình biết vai trò và thao tác đang muốn thực hiện.'}
...
export function AssistantProvider({children}:{children:React.ReactNode}){
  ...
  const showAssistant=pathname!=='/'&&pathname!=='/login'

  return <AssistantContext.Provider value={{theme,toggleTheme,open,setOpen}}>
    {children}

    {showAssistant&&<button type="button" className={`${styles.edgeTab}${open?` ${styles.edgeTabOpen}`:''}`} onClick={()=>setOpen(value=>!value)} aria-expanded={open} aria-controls="assistant-drawer" aria-label={open?'Đóng trợ lý':'Mở trợ lý hướng dẫn'}>
      <DoctorMascot/>
      <i aria-hidden="true"/>
    </button>}
    ...
    {showAssistant&&open&&<><button type="button" className={styles.drawerBackdrop} aria-label="Đóng trợ lý" onClick={()=>setOpen(false)}/>
    <section id="assistant-drawer" className={`${styles.chat} ${v2.chat} ${styles.chatOpen}`} data-assistant="panel" role="dialog" aria-modal="false" aria-label="Trợ lý hướng dẫn VNutriCare">
      <header className={v2.header}><div className={`${styles.botMark} ${v2.botMark}`}><DoctorMascot/><i/></div><div><small className={v2.eyebrow}>BÁC SĨ DINH DƯỠNG ẢO</small><strong>Trợ lý VNUTRICARE</strong><span><i/> Đang trực tuyến</span></div><button type="button" onClick={()=>setOpen(false)} aria-label="Đóng trợ lý"><Icon name="close"/></button></header>
      ...
      <div className={`${styles.messages} ${v2.messages}`} data-assistant="messages" aria-live="polite">
        ...
        {messages.map(message=><div key={message.id} data-from={message.from} className={`${message.from==='assistant'?styles.assistant:styles.user} ${v2.message}`}>{message.from==='assistant'&&<span><DoctorMascot/></span>}<div><small>{message.from==='assistant'?'Trợ lý VNUTRICARE':'Bạn'}</small><p>{message.text}</p></div></div>)}{typing&&<div className={`${styles.assistant} ${v2.message}`} data-from="assistant"><span><DoctorMascot/></span><div><small>Đang soạn câu trả lời</small><p className={styles.typing}><i/><i/><i/></p></div></div>}<div ref={endRef}/>
      </div>
