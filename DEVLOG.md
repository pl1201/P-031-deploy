...
- **Chưa làm (cố ý):** `dislikes`/`weight_goal` trong CSV không có cột DB tương ứng — bỏ qua, không mở migration mới ngoài phạm vi; `medications` của set2/set4 là NHÃN NHÓM THUỐC (`oral_antidiabetic`, `insulin`), không phải tên thuốc cụ thể — sẽ không khớp được `drug_food_interactions.csv`.
