...
### `DAT-26` ⏳ 263 món trích từ ViFoodRec — CÓ DỮ LIỆU, CHƯA MERGE, CHỜ LICENSE + R2
**Owner:** R2 · **P2** · **Deps:** DAT-24 (dùng lại `crawl_mnmn_dishes.py` làm khuôn mẫu)
...
**⚠️ Phát hiện phụ, KHÔNG sửa trong ticket này:** `src/clinical/seeds.py::load_vn_dishes()` hiện KHÔNG gate theo `verified_by` — món `pending` vẫn được CP-SAT dùng ngay nếu đã nằm trong `dishes.csv`. Safety net thực tế chỉ dựa vào so khớp chuỗi con `"R2 cần rà"`/`"THIẾU"`/`"CHƯA GHÉP"` trong cột `note`, và note của cả `dishes.mnmn.csv` lẫn `dishes.vifoodrec.csv` hiện KHÔNG chứa đúng các chuỗi đó → nếu gộp vào `dishes.csv` trong tương lai mà không sửa `load_vn_dishes()` trước, món chưa duyệt sẽ vào thẳng bộ giải mà không ai biết. Cần ticket riêng (đề xuất: sửa `load_vn_dishes()` gate cứng theo `verified_by != "pending"`) trước khi bất kỳ nguồn `pending` nào (kể cả mnmn hiện có) được merge vào `dishes.csv`.
**AC:** Không merge vào `dishes.csv` chính tới khi có (1) xác nhận license và (2) R2 duyệt từng món · Không tự đổi `verified_by` sang trạng thái đã duyệt.
