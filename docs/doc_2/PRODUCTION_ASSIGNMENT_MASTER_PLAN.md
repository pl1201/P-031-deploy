...
| Tài liệu | Trả lời câu hỏi | Chủ |
|---|---|---|
| [`PRODUCTION_READINESS_MASTER_PLAN.md`](./PRODUCTION_READINESS_MASTER_PLAN.md) | **Làm gì và vì sao** — bảy chặng, sáu quyết định kiến trúc, tiêu chí sẵn sàng | R1 |
| [`PATIENT_EXPERIENCE_MASTER_PLAN.md`](./PATIENT_EXPERIENCE_MASTER_PLAN.md) | **Tính năng trông ra sao** — đặc tả F1–F8, sơ đồ, ràng buộc an toàn | Bạn (R4) |
| [`docs/KE_HOACH_TRI_THUC_LAM_SANG_R2.md`](./docs/KE_HOACH_TRI_THUC_LAM_SANG_R2.md) | **Nội dung lâm sàng nào là đúng, lấy từ đâu, ai duyệt** | **R2** |
| **Tài liệu này** | **Ai làm, khi nào, và làm sao biết đã xong** | Bạn (R4) |

> 📌 **Cập nhật 17/08/2026 (R2):** bổ sung tài liệu thứ tư sau khi R2 tổng hợp hội thảo
> *"Điều trị cá thể hoá cho người bệnh Đái tháo đường"* (Hà Nội, 16/08 — 7 báo cáo, 129 trang).
> Nguồn đầy đủ ở [`docs/HOI_THAO_TDNA_DSF_2026-08-16.md`](./docs/HOI_THAO_TDNA_DSF_2026-08-16.md).
> Các dòng đánh dấu 🆕 bên dưới là phần R2 thêm — **R1 và R4 cần rà lại phần chạm vùng tệp của mình**
> (cụ thể: `C3-AGT-03` chạm `src/agents/optimizer.py` của R1; `C2-BE-02` chạm schema của R4).
...
- 🆕 **Kiêm nguồn tri thức lâm sàng:** theo dõi hội thảo/guideline mới, tổng hợp thành tài liệu đối chiếu được, và **đặc tả** phần cần người khác cài đặt. R2 giữ `docs/HOI_THAO_*.md` và `docs/KE_HOACH_TRI_THUC_LAM_SANG_R2.md`.
...
- **Không làm:** không viết pipeline/script (của R3); không viết code ứng dụng (của bạn). 🆕 Kể cả khi phát hiện khoảng trống lâm sàng nằm trong `optimizer.py` hay schema — R2 **đặc tả rồi bàn giao**, không tự sửa vùng tệp của người khác.
- **Cột mốc:** CP2 ký hai release · CP3 xác nhận ba phương án CP-SAT hợp lý lâm sàng 🆕 *(gồm cả phân bố năng lượng theo bữa, không chỉ tổng ngày)* · CP6 xác nhận nội dung trích dẫn RAG không sai lệch · CP9 kết luận lâm sàng dựa trên số liệu shadow mode.
...
| 🆕 `C1-CLN-02` | **Tổng hợp hội thảo t-DNA/DSF 16/08** — 7 báo cáo, 129 trang | R2 | ✅ **Xong** — `docs/HOI_THAO_TDNA_DSF_2026-08-16.md` |
| 🆕 `C1-CLN-03` | **Kế hoạch tri thức lâm sàng** — 17 khoảng trống, phân định ai làm gì | R2 | ✅ **Xong** — `docs/KE_HOACH_TRI_THUC_LAM_SANG_R2.md` |
| 🆕 `C1-CLN-04` | **Bàn giao 3 đặc tả cho R1 và R4** (phân bố bữa · trường xét nghiệm · chẩn đoán dinh dưỡng PES) — trước 12:00 ngày đầu Chặng 2, cùng kỷ luật đã áp cho R3 ở §2 | R2 | `docs/KE_HOACH_TRI_THUC_LAM_SANG_R2.md` §6 |
...
- 🆕 R1 và R4 đã **nhận và phản hồi** ba đặc tả `C1-CLN-04` của R2 — không cần đồng ý ngay, nhưng phải trả lời có làm hay không để R2 biết đường lập kế hoạch Chặng 2
...
| Mã | Việc | Người | Tệp |
|---|---|:-:|---|
| `C2-CLN-01` | Verify từng clinical rule, **rủi ro cao nhất trước**: natri, kali, protein cho CKD, tương tác thuốc mức `high`. 🆕 **Đã có bảng đối chiếu sẵn** từ hội thảo 16/08 — 10 rule, trong đó **6 kết luận "giữ nguyên"**, 1 điểm lệch cần quyết (protein %E: dự án 15–20 vs hội thảo 20–25) | R2 | `data/seeds/clinical_rules.csv` · bảng ở `KE_HOACH_TRI_THUC_LAM_SANG_R2.md` §4 |
| `C2-CLN-02` | **Data release món Việt đầu tiên** — mục tiêu 24 món thật sạch, đủ vai trò tinh bột, đạm, rau, canh, bữa phụ. 🆕 **Thêm 3 yêu cầu** (đo được 17/08): ① bổ sung món cá vai trò `protein` — hiện 9 món cá nhưng **6/9 là canh** nên không đạt "cá ≥ 2–3 bữa/tuần" theo nghĩa cá là đạm chính · ② loại **2 cặp món trùng tên khác nguồn** (`Canh chua cá`, `Cá kho tộ`) đang lách được ràng buộc không-lặp-món · ③ ghi chú hệ đơn vị vào `food_exchange_units.csv` | R2 | `data/seeds/**` |
| `C2-CLN-03` | Ký rule release và data release. **Không có chữ ký của R2 thì không rule nào được chạm bệnh nhân**. 🆕 Biên bản ký phải dẫn được nguồn đã đối chiếu (gồm hội thảo 16/08) để sau này truy được vì sao một ngưỡng được giữ hay đổi | R2 | `docs/` |
| 🆕 `C2-BE-02` | **Thêm trường dữ liệu lâm sàng còn thiếu** theo đặc tả R2 (`KE_HOACH_TRI_THUC_LAM_SANG_R2.md` §6.2): **HbA1c** (ưu tiên cao — là bước 1 của thuật toán t-DNA, thiếu nó thì không phân tầng được mục tiêu điều trị), vòng eo, FPG/PPG/LDL-C. R2 đề xuất dùng `patient_observations` (đã tồn tại) thay vì thêm cột hồ sơ, vì các chỉ số này cần **lịch sử theo thời gian**. ⚠️ Giai đoạn đầu **chỉ hiển thị + phân tầng cảnh báo**, KHÔNG để hệ thống tự đặt mục tiêu điều trị (ranh giới `CLAUDE.md` §3) | Bạn | `src/db/`, `alembic/` |
...
**CP2 — bằng chứng:** R2 đã ký hai release · generator chỉ đọc dữ liệu thuộc release đang hoạt động · R3 chứng minh bằng số liệu cụ thể là dữ liệu chưa duyệt **không** lọt được vào luồng bệnh nhân · rule chưa chắc chắn đã bị vô hiệu hoá chứ không để `to_verify` mà vẫn chạy · 🆕 bộ 24 món có **ít nhất 3 món cá vai trò `protein`** (không tính canh) và **không còn cặp món trùng tên**.
...
| `C3-CLN-01` | Kiểm tra ba phương án sinh ra có hợp lý lâm sàng không, không chỉ hợp lệ về số. 🆕 **Thêm tiêu chí:** kiểm cả **phân bố năng lượng theo bữa**, không chỉ tổng ngày | R2 | `eval/` |
| `C3-DAT-01` | **Khảo sát nguồn hướng dẫn lâm sàng** để chuẩn bị ingest vào `guideline_chunks` — ADA, KDIGO... bản nào có sẵn, định dạng gì, cấp phép dùng lại thế nào. Đầu ra là danh sách nguồn đã duyệt được với R2. 🆕 **R2 đã đề xuất sẵn bộ nguồn xếp theo rủi ro pháp lý** (`KE_HOACH_TRI_THUC_LAM_SANG_R2.md` §5) — QĐ 5481 ưu tiên 1 (văn bản pháp quy, rủi ro thấp); **tài liệu hội thảo 16/08 xếp 🔴 KHÔNG ingest** | R3 | `docs/` |
| 🆕 `C3-AGT-03` | **Ràng buộc phân bố năng lượng theo bữa** (sáng 20–25% · trưa 30–35% · phụ 5–10% · tối 30–35%). **Lý do lâm sàng:** hội thảo coi *carb timing* ngang hàng *quality*/*quantity* (ADA 2026); ca lâm sàng 3 là phản ví dụ thật — HbA1c 9,1% **nhưng glucose đói 3,6 mmol/L**, tức thực đơn "đạt tổng ngày" vẫn gây hạ đường huyết (HR tử vong 1,8). ⚠️ **Phải khác cách làm của AGT-09 đã hỏng:** vẫn giải MỘT model cả ngày, thêm ràng buộc **dải mềm** trên tổng kcal từng slot, đặt ở mức P1 để dùng được cơ chế nới ngưỡng mềm (DEC-075). **Đo trước khi chốt** — nếu > 2/12 hồ sơ eval vô nghiệm thì nới dải. **R1 có quyền kết luận "không nên làm"** nếu đo thấy vỡ tính khả thi | R1 | `src/agents/optimizer.py` · đặc tả ở `KE_HOACH_TRI_THUC_LAM_SANG_R2.md` §6.1 |
...
| `C4-CLN-01` | Định nghĩa care status: `insufficient_data`, `stable`, `watch`, `review_required`, `red_flag`. 🆕 **Bổ sung:** care status phải phân biệt được *chẩn đoán bệnh* với **chẩn đoán dinh dưỡng** (khung PES — Problem/Etiology/Signs). Hội thảo nhấn: *"đích can thiệp của MNT là chẩn đoán dinh dưỡng, không phải chẩn đoán bệnh"*. Ví dụ ca 3: `red_flag` không phải vì "HbA1c cao" mà vì **dao động đường huyết + hạ ĐH** | R2 | `src/clinical/**` |
| 🆕 `C4-BE-08` | **Ba trường PES trong `clinical_notes`** (Problem · Etiology · Signs) thay vì một ô text tự do — để chẩn đoán dinh dưỡng ghi được có cấu trúc. Đây là **nội dung chuyên gia nhập**, hệ thống không tự suy ra | Bạn | `src/db/`, `alembic/` |
...
| **RAG ingest sai nguồn hoặc thiếu giấy phép sử dụng lại** | Pipeline chạy được nhưng nội dung trích dẫn không rõ nguồn gốc pháp lý | `C3-DAT-01` bắt buộc R3 và R2 cùng duyệt danh sách nguồn **trước khi** viết pipeline ingest ở `C4-DAT-01`, không ingest rồi mới hỏi. 🆕 R2 đã xếp hạng sẵn — **tài liệu hội thảo 16/08 là 🔴 KHÔNG ingest** (có tên tác giả, logo đơn vị, và phần DSF dẫn nghiên cứu tài trợ bởi hãng) |
...
| 🆕 **Trợ lý quảng cáo sản phẩm thương mại gián tiếp** | Thực đơn hoặc câu trả lời của trợ lý nhắc tên một sản phẩm dinh dưỡng y tế (DSF/Glucerna...) mà bệnh nhân chưa khai đang dùng | Ranh giới **đã có sẵn và đã kiểm là đúng** (`src/clinical/medical_nutrition.py`: fail closed, chỉ vào thực đơn khi bệnh nhân khai dùng thật). **Không nới.** Hội thảo có phần DSF hấp dẫn nhưng mức bằng chứng chỉ grade C–D và có xung đột lợi ích — gợi ý sản phẩm bệnh nhân chưa dùng là tiến gần tới chỉ định (`CLAUDE.md` §3) |
| 🆕 **Trộn hai hệ đơn vị chuyển đổi thực phẩm** | Có dòng mới trong `food_exchange_units.csv` ghi trái cây 15 g glucid (hệ hội thảo) thay vì 10 g (hệ Viện Dinh dưỡng đang dùng) | Lệch tới **50%** ở nhóm trái cây và đạm. `C2-CLN-02` ③ ghi chú hệ đơn vị vào đầu file; đề xuất thêm kiểm tra vào `validate_data.py` |
| 🆕 **Thêm HbA1c rồi hệ thống tự đặt mục tiêu điều trị** | Có code đọc HbA1c để đổi ngưỡng dinh dưỡng tự động | Đặt mục tiêu HbA1c cho một bệnh nhân là **quyết định của bác sĩ điều trị**, không phải của phần mềm dinh dưỡng (`CLAUDE.md` §3). Giai đoạn đầu HbA1c chỉ để **hiển thị + phân tầng cảnh báo** |
...
| 🆕 **`optimizer.py` không có ràng buộc nào về phân bố năng lượng/carb theo bữa** — chỉ ràng buộc tổng cả ngày (cố ý từ AGT-10, vì bản AGT-09 chia tỉ lệ cố định rồi giải từng bữa đã gây vô nghiệm) | Thực đơn đạt mọi ngưỡng ngày vẫn có thể dồn carb lệch bữa. Hội thảo 16/08 coi *carb timing* là 1/3 trụ cột. → `C3-AGT-03` |
| 🆕 **`PatientProfile` không có HbA1c** và không có dữ liệu xét nghiệm nào (FPG, LDL-C, vòng eo) | "Cá thể hoá" hiện dừng ở nhân trắc + bệnh đồng mắc. Không phân tầng được mục tiêu điều trị (<7,0% vs <7,5%) — vốn là **bước 1** của thuật toán t-DNA. → `C2-BE-02` |
| 🆕 **9 món cá đã duyệt, nhưng 6/9 là *canh*** (cá là nguyên liệu phụ), chỉ 2 món vai trò `protein` | Không đạt được khuyến nghị "cá ≥ 2–3 bữa/tuần" theo nghĩa cá là nguồn đạm chính của bữa. → `C2-CLN-02` ① |
| 🆕 **2 cặp món trùng tên khác nguồn** (`Canh chua cá`, `Cá kho tộ`) | Ràng buộc không-lặp-món của CP-SAT lọc theo `dish_id` nên hai dòng này lách được — bệnh nhân có thể nhận cùng một món ở cả trưa lẫn tối. Cùng loại bug đã xử lý cho biến thể `-GVTT-` (DEC-052). → `C2-CLN-02` ② |
| 🆕 `src/clinical/medical_nutrition.py` **đã đúng sẵn** nguyên tắc DSF của hội thảo (*"không cộng thêm mà thay thế, tính vào tổng kcal"*) — fail closed, chỉ vào thực đơn khi bệnh nhân khai dùng thật | Không cần làm gì. Ghi lại để không ai "sửa" nhầm khi đọc tài liệu hội thảo thấy DSF được khuyến nghị mạnh |
| 🆕 **Không rule nào của dự án SAI** khi đối chiếu hội thảo — 6/10 dòng kết luận "giữ nguyên"; natri 2.000 mg còn **chặt hơn** khuyến nghị 2.300 mg | Bộ rule hiện tại vững hơn dự tính. Việc của `C2-CLN-01` chủ yếu là **ghi lại bằng chứng đã đối chiếu** vào `guideline_ref`, không phải sửa số |
